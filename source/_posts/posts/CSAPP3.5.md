...
![image-20201015164049061](../../images/image-20201015164049061.png)

左面是进行了一些复杂表达式的运算，最后把运算的结果rval变量作为返回值返回。
传入的参数有3个参数，x, y, z ,通过编译器可以将这段高级语言的语句翻译成汇编语言的指令. 

![image-20201015164449186](../../images/image-20201015164449186.png)
...
![image-20201015165930299](../../images/image-20201015165930299.png)
...
第一个特征就是**c 语言中的语句顺序和编译后生成的汇编指令的顺序不一定是完全一样的**。也就是指令在执行过程中顺序并不是c 语言中的逻辑顺序, 但最终运行结果是相同的, 编译器在这个过程中会打乱顺序。
...
![image-20201015181444447](../../images/image-20201015181444447.png)
...
> 背景知识;
>
> 在x86614位处理器上，如果执行一个32位的指令，意味着最后操作的目标寄存器的高32位会被清0。
